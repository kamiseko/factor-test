...
filenameICWeight = 'total_Weighted_By_P_Value.csv'  # Weight dataframe
...
stDF = pd.read_csv(path+filenameST, infer_datetime_format=True,parse_dates=[0],index_col=0).iloc[-timeStampNum:]

tradeDayDF = pd.read_csv(path+filenameTradeday, infer_datetime_format=True,parse_dates=[0],index_col=0).iloc[-timeStampNum:]

stopFlagDF = pd.read_csv(path+filenameStopFlag, infer_datetime_format=True,parse_dates=[0],index_col=0).iloc[-timeStampNum:]

benchMarkData = pd.read_csv(path+filenameHS300, infer_datetime_format=True, parse_dates=[0], index_col=0).iloc[-timeStampNum:]


# Own factors
DDA20df = pd.read_csv(path+filenameDDA20, infer_datetime_format=True, parse_dates=[0], index_col=0).iloc[-timeStampNum:]

Volatilitydf = pd.read_csv(path+filenameOwnVol, infer_datetime_format=True, parse_dates=[0], index_col=0).iloc[-timeStampNum:]

OwnILLQdf = pd.read_csv(path+filenameOWNILLIQ, infer_datetime_format=True, parse_dates=[0], index_col=0).iloc[-timeStampNum:]

IDIVOLdf = pd.read_csv(path+filenameIDIVOL, infer_datetime_format=True, parse_dates=[0], index_col=0)

ADJTOV20df = pd.read_csv(path+filenameADJTOV20, infer_datetime_format=True, parse_dates=[0], index_col=0)
...
def newToFilter(data, threshold=0.2, length=20):
...
def initial(sdk):
    ...
    totalstkList = sdk.getStockList()

    DDA20df.columns = totalstkList
    Volatilitydf.columns = totalstkList
    OwnILLQdf.columns = totalstkList
    IDIVOLdf.columns = totalstkList

    stDF.columns = totalstkList
    tradeDayDF.columns = totalstkList
    stopFlagDF.columns = totalstkList
    ADJTOV20df.columns = totalstkList
...
def initPerDay(sdk):
    ...
    if today in dateList:  # judge whether today is the last day of the month
        ...
        totalStkList = sdk.getStockList()
        suspendList = ff.GetSTNewSuspend(today, stDF, tradeDayDF, stopFlagDF)
        preparePool = list(set(totalStkList) - set(suspendList))
        ...
        # PBData
        PBDF = pd.DataFrame(np.array(sdk.getFieldData('LZ_GPA_VAL_PB')[-20:]), columns=totalStkList)
        #PBDF = PBDF[stockPool].fillna(method='ffill').fillna(method='bfill').iloc[-1]
        #PBSlice = (PBDF - PBDF.mean())/PBDF.std()  # normalize
        # OCFTODEBT Data
        OCFTODEBT = pd.DataFrame(np.array(sdk.getFieldData('LZ_GPA_FIN_IND_OCFTODEBT')[-20:]), columns=totalStkList)
        ...
        # MOM1MData
        MOM1MDF = pd.DataFrame(np.array(sdk.getFieldData('LZ_GPA_DERI_Momentum_1M')[-20:]), columns=totalStkList)
        #MOM1MDF = MOM1MDF[stockPool].fillna(method='ffill').fillna(method='bfill').iloc[-1]
        #MOM1MSlice = (MOM1MDF - MOM1MDF.mean())/MOM1MDF.std()
        # YOYGRData
        YOYGRDF = pd.DataFrame(np.array(sdk.getFieldData('LZ_GPA_FIN_IND_QFA_YOYGR')[-20:]), columns=totalStkList)
        #YOYGRDF = YOYGRDF[stockPool].fillna(method='ffill').fillna(method='bfill').iloc[-1]
        #YOYGRSlice = (YOYGRDF-YOYGRDF.mean())/YOYGRDF.std()
        # ILLIQData
        #ILLIQDF = pd.DataFrame(np.array(sdk.getFieldData('LZ_GPA_DERI_ILLIQ')[-20:]), columns=totalStkList)
        ...
        # Size - Adjusted turnover rate Volatility
        ADJTOV20 = ADJTOV20df.loc[:today]
        ADJTOV20 = ADJTOV20.iloc[-20:]
        ...
        factorList = [PBDF, OCFTODEBT, MOM1MDF, YOYGRDF, DDA20, VOL, OwnILLQ, IDIVOL, ADJTOV20]  # factorList
        factorList = [i[preparePool] for i in factorList]
        factorList = map(newToFilter, factorList)  # New method to filter data
        ...
        factorList = map(lambda x: x[stkList], factorList)  # get the intersection part
        SliceList = map(lambda x: x.iloc[-1], factorList)   #  Slice
        SliceList = map(lambda x: (x - x.mean()) / x.std(), SliceList)  # Normalize
        factorName = ['PB', 'OCFTODEBT', 'MOM_1M', 'YOYGR', 'DDA20', 'VOLATILITY', 'OWNILLIQ', 'IDIVOL', 'ADJTOV20']
        factorNamedict = dict(zip(factorName, range(len(factorName))))

        WeightSlice = factorWeight.loc[today]  # WeightSlice

        finalIndictor = pd.Series(data=0.0, index=totalStkList)
        for i, j in factorNamedict.iteritems():
            finalIndictor = finalIndictor + WeightSlice[i] * SliceList[j]
        ...
        ClosePriceDF = pd.DataFrame(CPData, columns=totalStkList)[stocksTobuy]
        ADfactor = np.array(sdk.getFieldData('LZ_GPA_CMFTR_CUM_FACTOR')[-121:])
        ADfactorDF = pd.DataFrame(ADfactor, columns=totalStkList)[stocksTobuy]
...
def main():
    config = {'username': 'toriphy', 'password': '1991toriphy', 'rootpath': 'D:/cStrategy/',
              ...
              'feeLimit': 5, 'strategyName': 'all(200stk)50+tryNew9fa-Nocutloss-MA12-15-cov120', 'logfile': 'ma.log', 'dealByVolume': True,
              'memorySize': 5,
